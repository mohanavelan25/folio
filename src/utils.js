...
export function sparkPath(id, positive, w = 60, h = 24) {
  ...
  const norm = pts.map(p => h - ((p - min) / (max - min + 0.01)) * (h - 4) - 2);
  return norm.map((y, i) => `${i === 0 ? 'M' : 'L'}${(i / (pts.length - 1)) * w},${y}`).join(' ');
}

/** Generate portfolio chart data for a given time range */
export function generateChartData(totalValue, range) {
  const points = { '1W': 7, '1M': 30, '3M': 90, '1Y': 252, 'ALL': 365 }[range] ?? 90;
  const data = [];
  const labels = [];
